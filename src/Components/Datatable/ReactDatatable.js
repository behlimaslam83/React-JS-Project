...
class ReactDatatable extends Component {
  ...
  constructor(props) {
    ...
    this.expandChild = this.expandChild.bind(this);
    this.config = {
      button: {
      ...
      language: {
        length_menu: (props.config && props.config.language && props.config.language.length_menu) ? props.config.language.length_menu : "Show _MENU_ entries",
  ...
  sortColumn(event, column, sortOrder) {
    // console.log(event, column, sortOrder,"TEST");
  ...
  // page_name: this.state.page_filter,
  onChange(){
    let tableData = {
      search_column: this.state.filter_type,
      search_value: this.state.filter_value,
    ...
    return (
      <div className="as-react-table" id={(this.props.id) ? this.props.id + "-container" : ""}>
        <TableHeader
          ...
          filterType={this.filterType.bind(this)}
        ...
        <div className="row table-body asrt-table-body"  id={(this.props.id) ? this.props.id + "-table-body" : ""}>
          <div className="col-md-12">
            <table className={this.props.className + ' addBrd'} id={this.props.id}>
              <thead className={this.props.tHeadClassName ? this.props.tHeadClassName : ''}>
                {
                  Array.from(Array(1), (e, i) =>{
                    var parentTh = [];
                    var childTh = [];
                    console.log(e,i, "LOOO TIMES");
                    this.props.columns.map((column, index) => {
                      ...
                      sortOrder = "";
                      let columnAscStyle = this.state.sortcolor.colorAscFill;
                      let columnDecStyle = this.state.sortcolor.colorDesFill;
                      if (column.sortable && this.state.sort.column === column.key) {
                        sortOrder = this.state.sort.order;
                        classText += (sortOrder) ? " " + sortOrder : "";
                        columnAscStyle = (sortOrder === "asc") ? 'blue' : columnAscStyle;
                        columnDecStyle = (sortOrder === "desc") ? 'blue' : columnDecStyle;
                      }
                      classText += " text-" + align;
                      if(column.TrOnlyClassName)
                        classText += " " + column.TrOnlyClassName;

                      if (column.className !=='childColumn'){
                        parentTh.push((<th
                          ...
                          {column.text}
                          {column.sortable && <div style={arrowTop}>
                            <svg style={arrowt}>
                              <path style={{ fill: columnAscStyle }} d="M3.5 3 L7 10 Q3.5 10 0 10z" />
                            </svg>
                            <svg style={arrowb}>
                              <path style={{ fill: columnDecStyle }} d="M3.5 7 L7 0 Q3.15 0 0 0z" />
                            </svg>
                          </div>}
                        </th>));
                      }else{
                    ...
                    let tablRowTr = (<tr>{parentTh}</tr>);
                    // let childRowTr = (<tr>{childTh}</tr>);
                    return (tablRowTr);
                  })
                }
              </thead>
