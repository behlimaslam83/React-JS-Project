...
class SocialMedia extends Component {
  ...
  render() {
    ...
    const columns = ['sr_no', 'social_scn_iso', 'social_icon', 'social_page_link', 'social_active_yn','actions'];
    const options = {
      perPage: PER_PAGE,
      headings: {
        sr_no: '#',
        social_scn_iso: 'Social ISO',
        social_icon: 'Icon',
        social_page_link: 'Link',
        social_active_yn: 'Active',
      ...
      sortable: ['social_scn_iso', 'social_page_link', 'social_active_yn'],
            ...
            <ServerTable renderView={this.state.renderTable} columns={columns} url={url} options={options} addme={$button} bordered hover updateUrl>
              {
                function (row, column, index) {
                  switch (column) {
                    ...
                    case 'social_icon':
                      return (
                        <img src={row.social_icon} width="50" className="table-image" alt="" />
                      );
                    case 'actions':
                      return (
                        <div className="form-control-sm" style={{ textAlign: 'center' }}>
                          <DropdownButton size="sm" id="dropdown-basic-button" title={<FontAwesomeIcon icon={faCog} />}>
                            <Dropdown.Item onClick={() => self.editRecord(row.social_id)}><FontAwesomeIcon icon={faEdit} /> Edit</Dropdown.Item>
                            <Dropdown.Item onClick={() => self.deletRecord(row.social_id)}><FontAwesomeIcon icon={faTrash} /> Delete</Dropdown.Item>
                          </DropdownButton>
                        </div>
                      );
                    default:
